...
class AppPanel extends HTMLElement {
  ...
  handleBodyTouchStart(event) {
    if (!this.open || !this.isDraggable() || event.touches.length !== 1) {
      this.bodyGesture = null;
      return;
    }
  ...
  setOffset(offset, options = {}) {
    ...
    this.sheetElement.style.transform = `translate3d(0, ${this.currentOffset}px, 0)`;
    ...
    const progress = this.computeOpenProgress();
    this.backdropElement.style.opacity = progress.toFixed(3);
    this.syncBackgroundEffect(progress);
  }
